...
4. Skip **Refresh WM** on the phone unless `OPENWEBNINJA_API_KEY` is set on Vercel

**GitHub:** https://github.com/sonya2233ff/royal-sass

## Walmart price source (RapidAPI / OpenWeb Ninja)

Shelf prices for store `#5831` can be pulled without Playwright:
...
2. Put `OPENWEBNINJA_API_KEY=...` in `.env` (and Vercel env for prod Refresh)
3. `npm run probe:walmart-rapid` — smoke search for staples at `#5831`
4. `WALMART_SOURCE=rapid npm run cache:walmart` — refresh local catalog
