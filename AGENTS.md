...
Shown staples = `PINNED_IDS` in `src/lib/staples.ts` plus `custom: true` items (`config/custom-staples.json`). Deleting a card removes it from `config/cafe-staples.json` and catalogs. Master product id is the cafe staple id (`simply_egg_whites`), never a PCX/Walmart SKU.
