...
- Globe eggplant (`eggplant_kg`) ≠ long / Chinese / Indian. Keep WM singles `6000191270102` and MVR 5 lb repack. NF/WC long eggplant → `no_match`.
- Fettuccine: No Name consumer packs at NF/WC stay. Catelli (WM) and De Cecco (MVR 500 g) are **ні**. Do **not** promote Granaria 4×5 lb / Only Pasta 5 kg warehouse cases.
- Fresh sliced white mushrooms ≠ canned Great Value (`6000197760572`). Keep Your Fresh Market 8 oz `6000196142668`.
...
Homepage search is **shown-catalog only** (`src/domain/staple-search.ts`): card label, id, queries, and include tokens — never retailer offer names, handles, or live WM/NF/WC/MVR hits. Searching `pumpkin` must not surface wraps because an NF foam pumpkin sat on that SKU. `яйця` / `eggs` hits only `large_eggs_dozen`. The homepage typeahead does **not** call `POST /api/staples/adopt` (match inspector). Empty catalog search (≥3 chars) can open **Додати продукт** and create a `custom: true` `custom_*` card after confirm. **Чек** (receipt photo / pasted text) can add `receipt_*` cards after the operator confirms new lines. Duplicate names (score ≥ 40) and shell eggs reuse the existing card — never a second egg line. Custom rows (`config/custom-staples.json` on a writable FS, and `localStorage` `royal-sass-custom-staples-v1` on Vercel) still show if present; do not commit that file unless asked. OCR matching a **deleted** cafe card counts as existing — do not auto-restore it.
...
- **Dissimilar pack mass/volume (every staple except eggs, loose kg, and supplies):** cover the largest pack on the row (−10% / +15%). Grape 283 g vs 907 g → **3×283 g**. Frozen 600 g vs 2 kg → **3×600 g**. Simply Egg Whites 500 ml vs 1 kg → **2×500 ml**. Similar packs stay 1-for-1. Homepage still does **not** inject 500 g when sizes match. Do not invent `maximumAmount`. Cups/lids stay count (12 oz is cup size).

Do not compare different pack masses as raw shelf prices (`src/domain/fair-compare.ts`). Checkout uses `sharedCoverGramsForDissimilarPacks` + `purchasePlanForPack` so the row winner is the cost to cover the same weight.
...
| `src/lib/waiter-compare.ts` | Catalog-only checkout for waiter send (no rematch) |
| `src/domain/whatsapp-list.ts` | Driver WhatsApp paste → shown Master Products (qty/unit, no new cards) |
| `src/lib/whatsapp-list-ai.ts` | Optional OpenAI id map for unsure lines; catalog ids only |
...
- `POST /api/driver/whatsapp-list` — paste a shopping list → shown Master Products only (qty/unit, merge duplicates). App auto-picks the best catalog hit. Count lines stay packs (`2 milk` ≠ 2 L). Mass lines (`tomato 5kg`) keep kg. Optional OpenAI maps leftover lines to catalog ids; never invents products or prices. Driver UI then stamps `POST /api/waiter/tickets` without a confirm step.
...
- Different pack masses → fair **$/100 g**, or **cover the largest pack** with N small packs (produce, frozen, dairy, juice — not eggs, not cups). Similar packs → per pack. Different OJ litres → `$/L` via that fair path (not `no_match`). Do **not** rank a 283 g clamshell vs a 907 g tub as one checkout pack each.
...
- Per-card **Видалити** and toolbar **Видалити вибрані** (cart ids), with confirm. Hidden via `royal-sass-removed-staples-v1`. Receipt `receipt_*` and homepage `custom_*` cards are also dropped from `royal-sass-custom-staples-v1`.
...
- **Додати продукт**: toolbar + empty search («Додати «…» як новий продукт»). Name required; optional extra query, A/B, Include/Exclude (pack-size tokens ignored). Duplicate / яйця → existing `large_eggs_dozen` (no second egg card). **Додати** = `custom_*` `no_match`. **Додати і знайти в магазинах** rematches **only that id**.
